...
//
//   - "Who changed this and when?" — [Service.Get] by id, or [Service.List]
//     filtered by subject/type/actor.
//   - "What did Alice do last week?" — [Service.List] with [ListInput.ActorSearch]
...
// ListInput controls a [Service.List] call. Zero value lists every audit log
...
// Iter returns a [iter.Seq2] over audit log events matching the
// supplied filters, fetching pages lazily on demand. Use this for
// large or potentially-unbounded result sets where buffering every
// match in memory ([Service.List]) is impractical.
//
...
// Example:
//
//	for ev, err := range svc.Iter(ctx, auditlogs.ListInput{TimeRangeStart: yesterday}) {
...
func (s *Service) Iter(ctx context.Context, input ListInput) iter.Seq2[AuditLog, error] {
	...
	return func(yield func(AuditLog, error) bool) {
		...
		for {
			resp, err := gen.ListAuditLogs(ctx, s.client.GQLv2, s.client.Config.OrganizationID, filter, sort, cursor)
			if err != nil {
				yield(AuditLog{}, gql.ClassifyError(fmt.Errorf("list audit logs: %w", err)))
				return
			}
...
// List returns audit log events matching the supplied filters,
// following pagination cursors automatically and buffering every
// match into a single slice.
//
// For large result sets — anything where the full match could be tens
// of thousands of rows — prefer [Service.Iter], which yields one event
// at a time. Cancel ctx to stop early.
func (s *Service) List(ctx context.Context, input ListInput) ([]AuditLog, error) {
	filter := buildListFilter(input)
	sort := buildListSort(input)
	...
	for {
		resp, err := gen.ListAuditLogs(ctx, s.client.GQLv2, s.client.Config.OrganizationID, filter, sort, cursor)
		if err != nil {
			return nil, gql.ClassifyError(fmt.Errorf("list audit logs: %w", err))
		}
		for _, item := range resp.AuditLogs.Items {
			a, derr := toAuditLog(item)
			if derr != nil {
